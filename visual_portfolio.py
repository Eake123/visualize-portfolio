...
# if own is set to False this needs to be filled. it has to be a list of the coins you want to plot
COINS = None
...
def multiply_market(dic,new_dic,new_market,owned=False):
    ...
    if new_market == True:
        os.remove(DIRECT + '\\marketvisual.txt')
    if owned == False:
...
def make_market_owned(just_add=False):
    ...
    while True:
        ...
        if a == '1':
            ...
            b = float(input('enter the amount of coins you have: '))
            c = coin(a)
            key = c.gecko_id()
            dic[key] = b
    J = Json('own_port.txt',DIRECT)
